...
(function (a) {
	...
	a.extend(a.jqx._jqxDropDownButton.prototype, {
		...
		render: function () {
			...
			if (this.touch) {
				this.addHandler(a(document), a.jqx.mobile.getTouchEventName("touchstart") + "." + this.element.id, b.closeOpenedDropDown, {
					me: this,
					popup: this.container,
					id: this.element.id
				})
			}
			this.addHandler(a(document), "mousedown." + this.element.id, b.closeOpenedDropDown, {
				me: this,
				popup: this.container,
				id: this.element.id
			});
				...
				if (parseInt(this.container.css("top")) != j.top) {
					...
					this.interval = setInterval(function () {
						if (b.outerHeight() != p.container.height()) {
							var r = p.testOffset(b, {
								left: parseInt(p.container.css("left")),
								top: parseInt(l)
							}, parseInt(p.host.outerHeight()));
		...
		closeOpenedDropDown: function (e) {
			...
			a.each(b.parents(), function () {
				if (this.className != "undefined") {
					if (this.className.indexOf("dropDownButton") != -1) {
						d = true;
						return false
					}
				}
