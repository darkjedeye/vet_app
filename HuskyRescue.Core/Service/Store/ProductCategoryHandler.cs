...
using ProductCategory = HuskyRescue.Core.ViewModel.Store.ProductCategory;
...
namespace HuskyRescue.Core.Service.Store
{
	public class ProductCategoryHandler : BaseHandler<ProductCategory>
	{
		...
		/// <summary>
		/// Add new ProductCategory to the database
		/// </summary>
		/// <param name="obj">ProductCategory object to be saved to the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Create(ref ProductCategory obj)
		{
			...
			try
			{
				...
				using (var context = new HuskyRescueEntities())
				{
					...
					dbObj = context.Entity_StoreCategory.Add(dbObj);
		...
		/// <summary>
		/// Updated existing ProductCategory in the database
		/// </summary>
		/// <param name="obj">ProductCategory object to be Updated in the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Update(ref ProductCategory obj)
		{
			// Number of changes as a result of the database change
			NumberChanges = 0;
			try
		...
		/// <summary>
		/// Delete an ProductCategory from the database
		/// </summary>
		/// <param name="id">ProductCategory id the object to be deleted from the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Delete(Guid id)
		{
			...
			try
			{
				...
				{
					// convert to database object
					var dbObj = context.Entity_StoreCategory.Find(id);

					if (dbObj != null)
					{
						context.Entity_StoreCategory.Remove(dbObj);
		...
		/// <summary>
		/// Retrieve on ProductCategory object for presentation
		/// </summary>
		...
		/// <returns>presentation ProductCategory object or null if not found</returns>
		public override ProductCategory ReadOne(Guid id, Guid id2, Guid id3, int id4, int id5, int id6)
		{
			...
			try
			{
				...
				using (var context = new HuskyRescueEntities())
				{
					...
					obj = context.Entity_StoreCategory
		...
		public override List<ProductCategory> ReadAll()
		{
			var objList = new List<ProductCategory>();
			try
			{
				// Perform data access using the context
				using (var context = new HuskyRescueEntities())
				{
					// convert to presentation object
					objList = context.Entity_StoreCategory
						.ToList().ToViewModel();
				}
			}
			catch (InvalidOperationException ex)
		...
		/// <returns>List of ProductCategory objects filtered and then sorted by name</returns>
		public override List<ProductCategory> ReadFiltered(ProductCategory obj)
		{
			var objList = new List<ProductCategory>();
			try
			{
				...
				var conditions = new List<Func<Model.Entity_StoreCategory, bool>>();
				...
				using (var context = new HuskyRescueEntities())
				{
					var query = context.Entity_StoreCategory.AsQueryable();
