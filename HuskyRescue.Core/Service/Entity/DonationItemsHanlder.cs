...
using DonationItems = HuskyRescue.Core.ViewModel.Entity.DonationItems;

namespace HuskyRescue.Core.Service.Entity
{
	public class DonationItemsHandler : BaseHandler<DonationItems>
	{
		public DonationItemsHandler()
		{
			ServiceResult = ServiceResultEnum.Failure;
		}

		/// <summary>
		/// Add new DonationItems to the database
		/// </summary>
		/// <param name="obj">DonationItems object to be saved to the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Create(ref DonationItems obj)
		{
			...
			try
			{
				...
				using (var context = new HuskyRescueEntities())
				{
					...
					dbObj = context.Entity_DonationItems.Add(dbObj);
		...
		/// <summary>
		/// Updated existing DonationItems in the database
		/// </summary>
		/// <param name="obj">DonationItems object to be Updated in the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Update(ref DonationItems obj)
		{
			// Number of changes as a result of the database change
			NumberChanges = 0;
			try
		...
		/// <summary>
		/// Delete an DonationItems from the database
		/// </summary>
		/// <param name="id">DonationItems id the object to be deleted from the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Delete(Guid id)
		{
			...
			try
			{
				...
				{
					// convert to database object
					var dbObj = context.Entity_DonationItems.Find(id);

					if (dbObj != null)
					{
						context.Entity_DonationItems.Remove(dbObj);
		...
		/// <summary>
		/// Retrieve on DonationItems object for presentation
		/// </summary>
		...
		/// <returns>presentation DonationItems object or null if not found</returns>
		public override DonationItems ReadOne(Guid id, Guid id2, Guid id3, int id4, int id5, int id6)
		{
			var obj = new DonationItems();
			try
			{
				// Perform data access using the context
				using (var context = new HuskyRescueEntities())
				{
					...
					obj = context.Entity_DonationItems.Find(id).ToViewModel();
				}
			}
			catch (InvalidOperationException ex)
			{
		...
		/// <summary>
		/// Retrieve all DonationItemses from the database for presentation
		/// </summary>
		/// <returns>list of DonationItems</returns>
		public override List<DonationItems> ReadAll()
		{
			...
			try
			{
				...
				{
					// convert to presentation object
					objList = context.Entity_DonationItems.ToList().ToViewModel();
				}
			}
			catch (InvalidOperationException ex)
			{
