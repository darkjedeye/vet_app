...
{
	public class GolfRegistrationHandler : NewBaseHandler, IBaseDabaseHandler<GolfRegistration>
	{
		public GolfRegistrationHandler()
		{
...
";
		...
		/// <returns>success or failure</returns>
		public ServiceResultEnum Update(ref GolfRegistration obj)
		{
			// Number of changes as a result of the database change
			NumberChanges = 0;
			try
		...
		public GolfRegistration ReadOne(Guid id, Guid id2, Guid id3, int id4, int id5, int id6)
		{
			var obj = new GolfRegistration();
			try
			{
				// Perform data access using the context
				using (var context = new HuskyRescueEntities())
				{
					...
					{
						case "golfevent":
							obj.EventRegistration.Event.IsActiveGolfEvent = true;
							break;
					}
		...
		public GolfRegistration ReadOne(Guid id)
		{
		...
		public List<GolfRegistration> ReadAll()
		{
			var objList = new List<GolfRegistration>();
			try
			{
				// Perform data access using the context
				using (var context = new HuskyRescueEntities())
				{
					...
					foreach (var item in objEventRegList)
					{
						...
						objList.Add(new GolfRegistration { EventRegistration = item });
					}
				}
			}
			catch (InvalidOperationException ex)
		...
		public List<GolfRegistration> ReadFiltered(GolfRegistration obj)
		{
			var objList = new List<GolfRegistration>();
			try
			{
