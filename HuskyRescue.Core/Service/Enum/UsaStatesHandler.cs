...
using UsaStates = HuskyRescue.Core.ViewModel.Enum.UsaStates;

namespace HuskyRescue.Core.Service.Enum
{
	public class UsaStatesHandler : BaseHandler<UsaStates>
	{
		public UsaStatesHandler()
		{
			ServiceResult = ServiceResultEnum.Failure;
		}

		/// <summary>
		/// Add new UsaStates to the database
		/// </summary>
		/// <param name="obj">UsaStates object to be saved to the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Create(ref UsaStates obj)
		{
			...
			try
			{
				...
				using (var context = new HuskyRescueEntities())
				{
					...
					dbObj = context.Enum_UsaStates.Add(dbObj);
		...
		/// <summary>
		/// Updated existing UsaStates in the database
		/// </summary>
		/// <param name="obj">UsaStates object to be Updated in the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Update(ref UsaStates obj)
		{
			// Number of changes as a result of the database change
			NumberChanges = 0;
			try
		...
		/// <summary>
		/// Delete an UsaStates from the database
		/// </summary>
		/// <param name="id">UsaStates id the object to be deleted from the database</param>
		/// <returns>success or failure</returns>
		public ServiceResultEnum Delete(string id)
		{
			...
			try
			{
				...
				{
					// convert to database object
					var dbObj = context.Enum_UsaStates.Find(id);

					if (dbObj != null)
					{
						context.Enum_UsaStates.Remove(dbObj);
		...
		/// <summary>
		/// Retrieve on UsaStates object for presentation
		/// </summary>
		...
		/// <returns>presentation UsaStates object or null if not found</returns>
		[Obsolete]
		public override UsaStates ReadOne(Guid id, Guid id2, Guid id3, int id4, int id5, int id6)
		{
			throw new Exception("Do not use this method");
		}
		...
		/// <param name="id"></param>
		/// <returns></returns>
		public UsaStates ReadOne(string id)
		{
			var obj = new UsaStates();
			try
			{
				// Perform data access using the context
				using (var context = new HuskyRescueEntities())
				{
					...
					obj = context.Enum_UsaStates.Find(id).ToViewModel();
				}
			}
			catch (InvalidOperationException ex)
			{
		...
		/// <summary>
		/// Retrieve all UsaStateses from the database for presentation
		/// </summary>
		/// <returns>list of UsaStates</returns>
		public override List<UsaStates> ReadAll()
		{
			var objList = new List<UsaStates>();
			try
			{
				// Perform data access using the context
				using (var context = new HuskyRescueEntities())
				{
					// convert to presentation object
					objList = context.Enum_UsaStates.ToList().ToViewModel();
				}
			}
			catch (InvalidOperationException ex)
			{
		...
		/// <returns>List of UsaStates objects filtered and then sorted by name</returns>
		public override List<UsaStates> ReadFiltered(UsaStates obj)
		{
			var objList = new List<UsaStates>();
			try
			{
				...
				var conditions = new List<Func<Enum_UsaStates, bool>>();
				...
				using (var context = new HuskyRescueEntities())
				{
					var query = context.Enum_UsaStates.AsQueryable();
