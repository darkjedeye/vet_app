...
using ResidenceType = HuskyRescue.Core.ViewModel.Enum.ResidenceType;

namespace HuskyRescue.Core.Service.Enum
{
	public class ResidenceTypeHandler : BaseHandler<ResidenceType>
	{
		public ResidenceTypeHandler()
		{
			ServiceResult = ServiceResultEnum.Failure;
		}

		/// <summary>
		/// Add new ResidenceType to the database
		/// </summary>
		/// <param name="obj">ResidenceType object to be saved to the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Create(ref ResidenceType obj)
		{
			...
			try
			{
				...
				using (var context = new HuskyRescueEntities())
				{
					...
					dbObj = context.Enum_ResidenceType.Add(dbObj);
		...
		/// <summary>
		/// Updated existing ResidenceType in the database
		/// </summary>
		/// <param name="obj">ResidenceType object to be Updated in the database</param>
		/// <returns>success or failure</returns>
		public override ServiceResultEnum Update(ref ResidenceType obj)
		{
			// Number of changes as a result of the database change
			NumberChanges = 0;
			try
		...
		/// <summary>
		/// Delete an ResidenceType from the database
		/// </summary>
		/// <param name="id">ResidenceType id the object to be deleted from the database</param>
		/// <returns>success or failure</returns>
		public ServiceResultEnum Delete(int id)
		{
			...
			try
			{
				...
				{
					// convert to database object
					var dbObj = context.Enum_ResidenceType.Find(id);

					if (dbObj != null)
					{
						context.Enum_ResidenceType.Remove(dbObj);
		...
		/// <summary>
		/// Retrieve on ResidenceType object for presentation
		/// </summary>
		...
		/// <returns>presentation ResidenceType object or null if not found</returns>
		[Obsolete]
		public override ResidenceType ReadOne(Guid id, Guid id2, Guid id3, int id4, int id5, int id6)
		{
			throw new Exception("Do not use this method");
		}
		...
		/// <param name="id"></param>
		/// <returns></returns>
		public ResidenceType ReadOne(int id)
		{
			var obj = new ResidenceType();
			try
			{
				// Perform data access using the context
				using (var context = new HuskyRescueEntities())
				{
					...
					obj = context.Enum_ResidenceType.Find(id).ToViewModel();
				}
			}
			catch (InvalidOperationException ex)
			{
		...
		/// <summary>
		/// Retrieve all ResidenceTypees from the database for presentation
		/// </summary>
		/// <returns>list of ResidenceType</returns>
		public override List<ResidenceType> ReadAll()
		{
			var objList = new List<ResidenceType>();
			try
			{
				// Perform data access using the context
				using (var context = new HuskyRescueEntities())
				{
					// convert to presentation object
					objList = context.Enum_ResidenceType.ToList().ToViewModel();
				}
			}
			catch (InvalidOperationException ex)
			{
		...
		/// <returns>List of ResidenceType objects filtered and then sorted by name</returns>
		public override List<ResidenceType> ReadFiltered(ResidenceType obj)
		{
			var objList = new List<ResidenceType>();
			try
			{
				...
				var conditions = new List<Func<Enum_ResidenceType, bool>>();
				...
				using (var context = new HuskyRescueEntities())
				{
					var query = context.Enum_ResidenceType.AsQueryable();
